...
class FmMemory:
    ...
    # -- 内存区域遍历（供模式扫描用） --------------------------
    def iter_regions(
        self, base=None, size=None, readable_only=True, writable_only=False, min_size=0
    ):
    ...
    def scan_pattern(
        ...
        hits = []
        for addr, rsize in self.iter_regions(
            base, size, writable_only=writable_only, min_size=min_size
        ):
            off = 0
            while off < rsize:
    ...
    def find_ascii(self, text, base=None, size=None, max_hits=64):
        ...
        return self.scan_pattern(text.encode("ascii"), base=base, size=size, max_hits=max_hits)
...
if __name__ == "__main__":
    ...
    pid = os.getpid()
    mem = FmMemory(
        pid, kernel32.OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, False, pid)
    )
    with mem:
        mods = mem.modules()
